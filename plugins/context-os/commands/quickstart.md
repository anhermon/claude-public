...
1. **Check if the bundled context-os CLI is available:** Run `context-os --help` (or `python3 /path/to/context-os-plugin/bin/context-os --help`). The CLI ships with the **context-os** plugin (`bin/context-os`). If it fails, tell the user to add the plugin `bin/` directory to `PATH` or invoke `python3 <plugin>/bin/context-os --help`, then run `/quickstart` again.
   **Stop here if CLI is not available.**
...
Read the template from `templates/CLAUDE_MD_STARTER.md` (in this plugin or the workspace root).
