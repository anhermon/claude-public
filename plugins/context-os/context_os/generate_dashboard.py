...
def slim_replay(replay) -> dict | None:
    ...
    for t in raw_turns:
        ...
        cc_u = float(usage.get("cache_creation_input_tokens") or 0)
        cr_u = float(usage.get("cache_read_input_tokens") or 0)
        ch_pct = round(cr_u / (cc_u + cr_u) * 100, 2) if (cc_u + cr_u) > 0 else None
        turns.append({
            "i": turn_idx,
...
WASTE_FIXES = {
    ...
    "tool_pollution":
        "Reduce total tool and MCP call volume: disable unused MCP servers, batch reads, "
...
def build_sessions_js(sessions):
    ...
    for s in sessions:
        sid = s.get("session_id", "")
        pch = s.get("per_turn_cache_hit_pct") or []
        slim = slim_replay(s.get("replay"))
        if slim and slim.get("turns") and pch:
            for i, t in enumerate(slim["turns"]):
                if i < len(pch):
                    t["ch"] = pch[i]
        result[sid] = {
            "session_id": sid,
            ...
            "repeat_read_ratio_pct": s.get("repeat_read_ratio_pct"),
            "sequential_tool_pct": s.get("sequential_tool_pct"),
            "has_compaction": s.get("has_compaction", False),
            "per_turn_cache_hit_pct": pch,
...
def generate_html(spec: dict) -> str:
    ...
    projects_donut = spec.get("projects_donut", [])
    ...
    projects_donut_json  = safe_json(projects_donut)
    ...
    for s in top_sessions_by_date:
        ...
        task = (s.get("first_user_message") or "").strip()
        task_display = (escape(task[:80]) + ("…" if len(task) > 80 else "")) if task else "—"
        task_title = escape(task) if task else ""
        ws_d = s.get("waste_scores") or {}
        badges_sorted = sorted(
            [(c, ws_d[c]) for c in ws_d if ws_d.get(c, 0) > 15],
            key=lambda x: -x[1],
        )[:3]
        badge_strip = "".join(
            f'<span style="color:{WASTE_COLORS.get(c, "#64748b")};border:1px solid {WASTE_COLORS.get(c, "#64748b")}44;'
            f'background:{WASTE_COLORS.get(c, "#64748b")}18;font-size:9px;padding:1px 4px;border-radius:4px;margin-left:3px">'
            f'{escape(WASTE_LABELS.get(c, c)[:18])} {scv}</span>'
            for c, scv in badges_sorted
        )
        compact_chip = ""
        if (s.get("duration_minutes") or 0) > 1000 and not s.get("has_compaction"):
            compact_chip = (
                '<span title="Session &gt;1000 min without /compact" '
                'style="font-size:10px;padding:2px 6px;border-radius:4px;'
                'background:#ef444433;color:#ef4444;border:1px solid #ef444444;margin-right:6px">⚠ no /compact</span>'
            )
        table_rows.append(
            ...
            f'<td>{escape(proj)}</td>'
            f'<td title="{task_title}" style="max-width:260px;font-size:12px;color:var(--text-muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{task_display}</td>'
            f'<td class="num">{cost}</td>'
            f'<td class="num">{dur}</td>'
            f'<td class="num">{cache}</td>'
            f'<td class="num">{tools}</td>'
            f'<td class="num" style="white-space:nowrap"><span class="badge {sc}">{ws}</span>{badge_strip}</td>'
            f'<td>{compact_chip}<span class="waste-tag" style="background:{tw_color}22;color:{tw_color};border:1px solid {tw_color}44">{tw_label}</span></td>'
            f'</tr>'
        )
    table_rows_html = "\n".join(table_rows)
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .dd-kpi-row {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 12px; margin-bottom: 20px; }}
    ...
    <table class="sessions" id="sessionsTable">
      <thead>
        <tr>
          ...
          <th onclick="sortTable(4)" class="num">Cost</th>
          <th onclick="sortTable(5)" class="num">Duration</th>
          <th onclick="sortTable(6)" class="num">Cache Hit%</th>
          <th onclick="sortTable(7)" class="num">Tools</th>
          <th onclick="sortTable(8)" class="num">Waste</th>
          <th onclick="sortTable(9)">Top Issue</th>
        </tr>
      </thead>
...
const PROJECTS_DONUT = {projects_donut_json};
...
function openDrilldown(sid) {{
  ...
  if (ddCacheSpark) {{ ddCacheSpark.destroy(); ddCacheSpark = null; }}
  ...
  // KPI cards
  const rr = s.repeat_read_ratio_pct;
  const sq = s.sequential_tool_pct;
  const rrCls = rr == null ? '' : (rr < 15 ? 'color:var(--green)' : rr < 30 ? 'color:var(--amber)' : 'color:var(--red)');
  const sqCls = sq == null ? '' : (sq < 40 ? 'color:var(--green)' : sq < 60 ? 'color:var(--amber)' : 'color:var(--red)');
  const kpiHTML = `
    <div class="dd-kpi-row">
      ...
      <div class="dd-kpi-card">
        <div class="kpi-label">Repeat read %</div>
        <div class="kpi-value" style="${{rrCls}}">${{rr == null ? '—' : rr.toFixed(1)+'%'}}</div>
        <div class="kpi-sub" style="font-size:10px;color:var(--text-muted)">&lt;15 green · &lt;30 amber</div>
      </div>
      <div class="dd-kpi-card">
        <div class="kpi-label">Sequential tools %</div>
        <div class="kpi-value" style="${{sqCls}}">${{sq == null ? '—' : sq.toFixed(1)+'%'}}</div>
        <div class="kpi-sub" style="font-size:10px;color:var(--text-muted)">&lt;40 green · &lt;60 amber</div>
      </div>
      <div class="dd-kpi-card">
  ...
  const findingCards = Object.entries(wasteScores)
    ...
    .map(([cat, score]) => {{
      ...
      const fix = ev.recommendation || WASTE_FIXES[cat] || '';
      return `
  ...
  const sparkArr = s.per_turn_cache_hit_pct || [];
  const sparkBlock = sparkArr.length > 0
    ? `<div style="margin-top:14px"><div style="font-size:11px;color:var(--text-muted);margin-bottom:6px">Cache read share per assistant turn (higher = more reuse)</div><div style="position:relative;height:90px"><canvas id="ddCacheSpark"></canvas></div></div>`
    : '';
  ...
  ddContent.innerHTML = `
    <div class="dd-header">
      <div class="dd-title">
        ...
        ${{s.first_user_message ? `<div style="font-size:12px;color:var(--text-muted);margin-top:8px;line-height:1.45;max-width:900px">${{escHtml(s.first_user_message)}}</div>` : ''}}
      </div>
  ...
  if (sparkArr.length > 0) {{
    const csp = document.getElementById('ddCacheSpark');
    if (csp) {{
      ddCacheSpark = new Chart(csp.getContext('2d'), {{
        type: 'line',
        data: {{
          labels: sparkArr.map((_, i) => 'T' + (i + 1)),
          datasets: [{{
            label: 'Cache read %',
            data: sparkArr,
            borderColor: '#22c55e',
...
// ── B-5: Donut chart — cost % by project (uses full project cost list, not waste-bar data)
(function() {{
  const canvas = document.getElementById('chartDonut');
  if (!canvas) return;
  let PD = PROJECTS_DONUT || [];
  if (!PD.length) {{
    const by = {{}};
    Object.values(SESSIONS).forEach(s => {{
      const k = s.project_name || 'unknown';
      by[k] = (by[k] || 0) + (Number(s.estimated_cost) || 0);
    }});
    PD = Object.entries(by).map(([label, cost]) => ({{ label, cost }}));
  }}
  const labels = PD.map(p => p.label || p.slug || '?');
  const costs  = PD.map(p => Number(p.cost) || 0);
...
function renderTurnTable(turns, containerId) {{
  ...
  let html = `<table class="turn-table">
    <thead><tr>
      ...
      <th class="num" style="width:52px">Cr%</th>
  ...
  turns.forEach((t, idx) => {{
    ...
    const chv = (t.ch != null && t.ch !== undefined) ? t.ch.toFixed(0) + '%' : '—';
    html += `<tr class="turn-row" id="turn-row-${{idx}}" onclick="toggleTurnRow(${{idx}})">
      ...
      <td class="num" style="font-size:11px;color:var(--text-muted)">${{chv}}</td>
    ...
    if (tcs.length > 0) {{
      html += `<tr class="turn-detail-row" id="turn-detail-${{idx}}" style="display:none">
        <td colspan="6"><div class="tool-calls-list">${{details}}</div></td>
      </tr>`;
    }}
  }});
