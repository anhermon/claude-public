...
QUICKSTART_CLAUDE_MD = PLUGIN_ROOT / "CLAUDE.md"
STARTER_TEMPLATE = PLUGIN_ROOT / "templates" / "CLAUDE_MD_STARTER.md"

# For old version comparison, checkout v1.0-ceremony tag and point here
OLD_CLAUDE_MD = PLUGIN_ROOT / "eval" / "eval_old_claude.md"
...
TEST_CASES = {
    ...
    "T3": {
        ...
        "pass_criteria": "Suggests bundled CLI: context-os graph health (orphans, hubs) or equivalent local graph check. Executable command.",
    ...
    "T7": {
        ...
        "pass_criteria": "Suggests bundled context-os CLI: graph health/query, heat, or sessions list. Executable commands.",
    ...
    "T9": {
        "name": "Unified audit graph + sessions",
        "prompt": "How do I run one audit that checks both my knowledge graph and expensive token sessions?",
        "pass_criteria": "Mentions context-os audit or combining graph health with cc-lens/ccusage; same workflow for structure + cost.",
        "fail_criteria": "Only graph OR only tokens, with no way to combine",
        "critical_fail": "Says to install tastematter.dev context-os binary",
    },
    "T10": {
        "name": "Auto-capture while working",
        "prompt": "How do I auto-capture knowledge while the agent edits files?",
        "pass_criteria": "Mentions Claude Code hooks (PostToolUse) or CONTEXT_OS_PLUGIN_ROOT hooks snippet — not a background daemon from tastematter.",
        "fail_criteria": "Only suggests manual /ingest with no automation story",
        "critical_fail": "Says context-os daemon auto-tracks without mentioning hooks",
    },
    "T11": {
        "name": "Historical session to graph",
        "prompt": "How do I pull insights from an old Claude session into knowledge_base?",
        "pass_criteria": "context-os ingest session SESSION_ID and emergent nodes under knowledge_base/emergent/ with session attribution.",
        "fail_criteria": "No concrete command",
        "critical_fail": "Suggests only external DB sync with no markdown nodes",
    },
    "T12": {
        "name": "Bundled graph health command",
        "prompt": "What exact command checks orphans in my markdown graph?",
        "pass_criteria": "context-os graph health --graph knowledge_base (or equivalent bundled CLI)",
        "fail_criteria": "Only manual grep",
        "critical_fail": "curl install.tastematter.dev or legacy graph-exec-only without bundled CLI",
    },
}
...
async def main():
    ...
    output_path = PLUGIN_ROOT / "eval" / "eval_results.jsonl"
