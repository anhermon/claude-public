...
**CLI note:** Run `context-os heat --format json` and interpret whatever fields the binary returns. The formulas below are **conceptual**; your build may expose a subset (e.g. days window, path filter) without every column listed in older examples.
...
```bash
# Typical invocation — adjust graph root via path filter
context-os heat --days 30 --path-contains "knowledge_base" --format json --limit 50
```

Inspect the JSON for paths, counts, or scores your build emits. Use the **Interpretation** tables above only when the corresponding metrics appear in output.
...
**Version:** 2.1 (CLI examples updated to `context-os heat`; conceptual model unchanged)
